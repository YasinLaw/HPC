...
int main(int argc, char *argv[]) {
  ...
  int64_t right = sieve_start + step * (rank + 1) > length
                      ? length
                      : sieve_start + step * (rank + 1);
  int64_t offset = left;
  int64_t count = 0;
  ...
  if (rank == source) {
    ...
    for (int i = 0; i < source; i++) {
      MPI_Recv(&task_count, 1, MPI_INT, i, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      count += task_count;
    }
    printf(
        "for this round we used %d processors, got %lld prime numbers "
        "between 0 and %lld\n",
        size, count, length);
    delete[] array;
  } else {
